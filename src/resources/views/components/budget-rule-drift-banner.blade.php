        ...
        @if ($detailed)
            ...
            <ul class="list-disc list-inside space-y-0.5">
                @if ($drift['mandatory_over'])
                    <li>Needs is <strong>{{ $ratios['mandatory'] }}%</strong> of income — over the 50% ceiling. Either income is too low for your fixed costs, or some expenses tagged mandatory may not really be.</li>
                @endif
                @if ($drift['savings_under'])
                    <li>Wealth building is <strong>{{ $ratios['savings'] }}%</strong> of income — under the 20% target. Try funding your emergency fund or investing envelope more each month.</li>
                @endif
            </ul>
        @else
            <div>
                ...
                @if ($drift['mandatory_over'])
                    Needs is {{ $ratios['mandatory'] }}% of income (target ≤ 50%).
                @endif
                @if ($drift['savings_under'])
