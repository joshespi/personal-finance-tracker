@php $m = $manualAsset ?? null; @endphp

<div>
    <x-input-label for="name" value="Asset Name" />
    <x-text-input id="name" name="name" type="text" class="mt-1 block w-full"
                  :value="old('name', $m?->name)" required autofocus maxlength="200"
                  placeholder="My 401k" />
...
<div>
    ...
    <select id="asset_class" name="asset_class"
        ...
        @foreach ($assetClasses as $value => $label)
            <option value="{{ $value }}" @selected(old('asset_class', $m?->asset_class) === $value)>{{ $label }}</option>
        @endforeach
    </select>
...
<div>
    ...
    <x-text-input id="cost_basis" name="cost_basis" type="number" class="mt-1 block w-48"
                  :value="old('cost_basis', $m?->cost_basis)" min="0" step="any" placeholder="200000.00" />
...
<div>
    ...
    <textarea id="description" name="description"
              class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm"
              rows="3" maxlength="1000">{{ old('description', $m?->description) }}</textarea>
...
<div>
    ...
    <x-text-input id="currency" name="currency" type="text" class="mt-1 block w-32"
                  :value="old('currency', $m?->currency ?? $portfolio->currency)" required maxlength="3"
                  placeholder="USD" style="text-transform:uppercase" />
    <x-input-error :messages="$errors->get('currency')" class="mt-2" />
</div>

<x-checkbox-field name="include_in_chart" label="Include in portfolio charts" :model="$m"
    help="Uncheck to exclude this asset from chart totals (e.g. primary residence)." />

<x-checkbox-field name="include_in_invested" label="Count as invested" :model="$m"
    help="Uncheck for assets you don't consider investments (e.g. your home). Still counts toward net worth." />
    ...
    <div x-show="tracking === 'proxy_ticker'" x-cloak
        ...
        <div>
            ...
            <x-text-input id="proxy_symbol" name="proxy_symbol" type="text" class="mt-1 block w-32"
                          :value="old('proxy_symbol', $m?->proxyAsset?->symbol)" maxlength="20" placeholder="IWB"
        ...
        <div class="flex flex-wrap gap-4">
            <div>
                ...
                <x-text-input id="anchor_value" name="anchor_value" type="number" class="mt-1 block w-40"
                              :value="old('anchor_value', $m?->anchor_value)" min="0" step="any" placeholder="50000.00" />
            ...
            <div>
                ...
                <x-text-input id="anchor_date" name="anchor_date" type="date" class="mt-1 block w-40"
                              :value="old('anchor_date', $m?->anchor_date?->format('Y-m-d') ?? now()->format('Y-m-d'))" />
